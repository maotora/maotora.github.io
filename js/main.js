...
var Sly = __webpack_require__(83);

$(document).ready(function () {
    var user_id = '2405689';
    var queryUrl = 'https://api.stackexchange.com/2.2/users/' + user_id + '?order=desc&sort=reputation&site=stackoverflow';

    $.ajax({ url: queryUrl }).done(function (data) {
        var _data$items$ = data.items[0],
            badge_counts = _data$items$.badge_counts,
            display_name = _data$items$.display_name,
            reputation = _data$items$.reputation,
            profile_image = _data$items$.profile_image,
            link = _data$items$.link;


        $('.so-profile-picture-container').prepend('<img src=' + profile_image + '/>');

        $('.so-display-name').append(display_name);

        $('.so-reputation').append(reputation);

        $('.so-badge-gold').siblings('.so-gold-count').append(badge_counts.gold);

        $('.so-badge-silver').siblings('.so-silver-count').append(badge_counts.silver);

        $('.so-badge-bronze').siblings('.so-bronze-count').append(badge_counts.bronze);

        $('.so-profile-link').append('<a href=' + link + ' target="_blank" />');
    });

    $('.copyrights p').append(new Date().getFullYear());

    $('.job_btn').click(function (e) {
        console.log('Maaaaaa........', Sly);
    });
});
